...
# ╔═╡ 964422e4-494b-4e6a-a7df-aa259a8cd93f
hilitecolor = :orange

# ╔═╡ a56fbde8-af00-47e0-b340-7c69e3e62fad
normalcolor = :black
...
# ╔═╡ 517bf4e0-8005-4545-8fe5-d662ba04a9a3
construction = Dict(
	[1 => "Describe the circle BCD with center A and radius AB.",
	 2 => "Again describe the circle ACE with center B and radius BA.",
	 3 => "Join the straight lines CA and CB from the point C at which the circles cut one another to the points A and B."

	])

# ╔═╡ 136f13fb-83e5-42ca-966f-2e5fcce4a398
constructionmenu =  vcat([0], collect(keys(construction))) |> sort
...
begin
	#plotA = scatter!(constructionaxis, a) 
	#plotB = scatter!(constructionaxis, b) 

	
	#lineAB = lines!(constructionaxis, radius)
	if constructionstep == 0 
		empty!(constructionaxis.scene)
	elseif constructionstep == 1
		scatter!(constructionaxis, a; color = hilitecolor)
		scatter!(constructionaxis, b; color = hilitecolor)
		lines!(constructionaxis, radius; color = hilitecolor)

	else
		scatter!(constructionaxis, a; color = normalcolor)
		scatter!(constructionaxis, b; color = normalcolor)
		lines!(constructionaxis, radius; color = normalcolor)
	end

end

# ╔═╡ bfe8312a-7a6b-49cd-b68f-145c0e9fb8d7
begin
	#labelA = text!(constructionaxis, a, text = "A")
	#labelB = text!(constructionaxis, b, text = "B")
	if constructionstep > 0
		text!(constructionaxis, a, text = "A") #rich("A", font = :regular, offset = (0.15, 0)))
		text!(constructionaxis, b, text = rich("B", font = :regular, offset = (0.15, 0)))
	end
end
...
proof = Dict([
	1 => "Now, since the point A is the center of the circle CDB, therefore AC equals AB.",
	2 => "Again, since the point B is the center of the circle CAE, therefore BC equals BA.",
	3 => "But AC was proved equal to AB, therefore each of the straight lines AC and BC equals AB.",
	4 => "And things which equal the same thing also equal one another, therefore AC also equals BC.",
	5 => "Therefore the three straight lines AC, AB, and BC equal one another."

])

# ╔═╡ 9513792d-5d07-43af-a210-21eea335f2f2
proofmenu =  vcat([0], collect(keys(proof))) |> sort
...
manifest_format = "2.0"
project_hash = "6aa9df3a5e3078e24f1e910b9fb105bc34dbcc65"
